...
$page_title = "Ribbons Receiving - SLPA System";
$additional_css = ['../assets/css/ribbons-receiving.css', '../assets/css/forms.css', '../assets/css/components.css'];
$additional_js = ['../assets/js/ribbons-receiving.js?v=' . time()];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'receive') {
    ...
    if (empty($ribbon_id) || empty($ribbon_model) || empty($receive_date) || ($jct_quantity <= 0 && $uct_quantity <= 0)) {
        $_SESSION['message'] = 'Please fill in all required fields and ensure at least one quantity is greater than 0!';
        $_SESSION['message_type'] = 'error';
    } else {
        // Validate that ribbon exists in ribbons_master
        try {
            $validate_stmt = $conn->prepare("SELECT ribbon_id, ribbon_model FROM ribbons_master WHERE ribbon_id = ?");
            $validate_stmt->bind_param("i", $ribbon_id);
            $validate_stmt->execute();
            $validate_result = $validate_stmt->get_result();
            $master_ribbon = $validate_result->fetch_assoc();
            $validate_stmt->close();
            
            if (!$master_ribbon) {
                $_SESSION['message'] = 'Error: Selected ribbon (ID: ' . $ribbon_id . ') does not exist in ribbons Master! Please add it to ribbons Master first.';
                $_SESSION['message_type'] = 'error';
                header('Location: ' . $_SERVER['PHP_SELF']);
                exit;
            }
            
            // Verify ribbon type matches
            if ($master_ribbon['ribbon_model'] !== $ribbon_model) {
                $_SESSION['message'] = 'Warning: ribbon type mismatch! Master has "' . $master_ribbon['ribbon_model'] . '" but you selected "' . $ribbon_model . '"';
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO ribbons_receiving (ribbon_id, ribbon_model, lot, stock, supplier_name, pr_no, tender_file_no, jct_quantity, uct_quantity, unit_price, invoice, remarks, receive_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("issssssiidsss", $ribbon_id, $ribbon_model, $lot, $stock, $supplier_name, $pr_no, $tender_file_no, $jct_quantity, $uct_quantity, $unit_price, $invoice, $remarks, $receive_date);
            
            if ($stmt->execute()) {
                $receive_id_new = $stmt->insert_id;
                
                // Update ribbons master stock
                $update_stmt = $conn->prepare("UPDATE ribbons_master SET jct_stock = jct_stock + ?, uct_stock = uct_stock + ? WHERE ribbon_id = ?");
                $update_stmt->bind_param("iii", $jct_quantity, $uct_quantity, $ribbon_id);
                
                if ($update_stmt->execute()) {
                    $affected_rows = $update_stmt->affected_rows;
                    if ($affected_rows > 0) {
                        $_SESSION['message'] = 'ribbon received successfully! Stock updated (JCT +' . $jct_quantity . ', UCT +' . $uct_quantity . ')';
                        $_SESSION['message_type'] = 'success';
                    } else {
                        $_SESSION['message'] = 'ribbon received but ribbon not found in master! Please check ribbon_id: ' . $ribbon_id;
                        $_SESSION['message_type'] = 'warning';
                    }
                } else {
                    $_SESSION['message'] = 'ribbon received but stock update failed: ' . $update_stmt->error;
                    $_SESSION['message_type'] = 'warning';
                }
                $update_stmt->close();
            } else {
                $_SESSION['message'] = 'Error receiving ribbon: ' . $stmt->error;
                $_SESSION['message_type'] = 'error';
            }
            $stmt->close();
        } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    // Validate required fields
    if (empty($receive_id) || empty($ribbon_id) || empty($ribbon_model) || empty($receive_date) || ($jct_quantity <= 0 && $uct_quantity <= 0)) {
        $_SESSION['message'] = 'Please fill in all required fields and ensure at least one quantity is greater than 0!';
        $_SESSION['message_type'] = 'error';
    } else {
        // Validate that ribbon exists in ribbons_master
        try {
            ...
            if (!$master_ribbon) {
                $_SESSION['message'] = 'Error: Selected ribbon (ID: ' . $ribbon_id . ') does not exist in ribbons Master! Cannot update.';
                $_SESSION['message_type'] = 'error';
                header('Location: ' . $_SERVER['PHP_SELF']);
                exit;
            }
        ...
        try {
            ...
            $orig_stmt = $conn->prepare("SELECT jct_quantity, uct_quantity, ribbon_id FROM ribbons_receiving WHERE receive_id = ?");
            ...
            if ($original) {
                ...
                $stmt = $conn->prepare("UPDATE ribbons_receiving SET ribbon_id = ?, ribbon_model = ?, lot = ?, stock = ?, supplier_name = ?, pr_no = ?, tender_file_no = ?, jct_quantity = ?, uct_quantity = ?, unit_price = ?, invoice = ?, remarks = ?, receive_date = ? WHERE receive_id = ?");
                $stmt->bind_param("issssssiiisssi", $ribbon_id, $ribbon_model, $lot, $stock, $supplier_name, $pr_no, $tender_file_no, $jct_quantity, $uct_quantity, $unit_price, $invoice, $remarks, $receive_date, $receive_id);
                
                if ($stmt->execute()) {
                    // Adjust stock - restore original quantities then add new quantities
                    $restore_stmt = $conn->prepare("UPDATE ribbons_master SET jct_stock = jct_stock - ?, uct_stock = uct_stock - ? WHERE ribbon_id = ?");
                    $restore_stmt->bind_param("iii", $original['jct_quantity'], $original['uct_quantity'], $original['ribbon_id']);
                    $restore_stmt->execute();
                    $restore_stmt->close();
                    
                    // Add new quantities
                    $add_stmt = $conn->prepare("UPDATE ribbons_master SET jct_stock = jct_stock + ?, uct_stock = uct_stock + ? WHERE ribbon_id = ?");
                    $add_stmt->bind_param("iii", $jct_quantity, $uct_quantity, $ribbon_id);
                    $add_stmt->execute();
                    $add_stmt->close();
                    
                    $_SESSION['message'] = 'ribbon receiving updated successfully!';
                    $_SESSION['message_type'] = 'success';
                } else {
                    $_SESSION['message'] = 'Error updating ribbon receiving: ' . $conn->error;
                    $_SESSION['message_type'] = 'error';
                }
                $stmt->close();
            } else {
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        $get_stmt = $conn->prepare("SELECT jct_quantity, uct_quantity, ribbon_id FROM ribbons_receiving WHERE receive_id = ?");
        ...
        if ($receiving) {
            // Delete the record
            $stmt = $conn->prepare("DELETE FROM ribbons_receiving WHERE receive_id = ?");
            $stmt->bind_param("i", $receive_id);
            
            if ($stmt->execute()) {
                // Restore stock
                $update_stmt = $conn->prepare("UPDATE ribbons_master SET jct_stock = jct_stock - ?, uct_stock = uct_stock - ? WHERE ribbon_id = ?");
                $update_stmt->bind_param("iii", $receiving['jct_quantity'], $receiving['uct_quantity'], $receiving['ribbon_id']);
                $update_stmt->execute();
                $update_stmt->close();
...
// Get ribbon receivings from database
try {
    $result = $conn->query("SELECT * FROM ribbons_receiving ORDER BY receive_date DESC, receive_id DESC");
    if ($result) {
...
// Get ribbons from database for dropdown - always get all ribbon types from ribbons_master
try {
    $result = $conn->query("SELECT ribbon_id, ribbon_model FROM ribbons_master ORDER BY ribbon_model");
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $ribbons[] = $row;
        }
        // Debug: Check if ribbons were loaded
        if (empty($ribbons)) {
            $_SESSION['message'] = 'No ribbons found. Please add ribbons to ribbons Master first.';
            $_SESSION['message_type'] = 'warning';
        }
    } else {
        ...
        <div class="header-content">
            <div class="header-text">
                <h1>
                    ...
                    Ribbons Receiving Management
                </h1>
                <p>Track and manage ribbon deliveries and stock receipts</p>
            </div>
            <div class="header-actions">
                <button class="btn btn-primary" onclick="openModal('receiveribbonModal')">
    ...
    <div class="content-card">
        ...
        <?php if (empty($receivings)): ?>
            <div class="no-data">
                ...
                <p>Start by receiving your first ribbon delivery.</p>
                <button class="btn btn-primary" onclick="openModal('receiveribbonModal')">
                    <i class="fas fa-plus"></i>
                    Receive First ribbon
                </button>
            </div>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($receivings as $receiving): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($receiving['ribbon_model']); ?></td>
                                <td>
                                ...
                                <td style="display: none;"><?php echo htmlspecialchars($receiving['stock'] ?: 'N/A'); ?></td>
...
<!-- Receive ribbon Modal -->
<div id="receiveribbonModal" class="modal">
    <div class="modal-content modal-large">
        <div class="modal-header">
            <h2><i class="fas fa-truck-loading"></i> Receive New ribbon</h2>
            <span class="modal-close" onclick="closeModal('receiveribbonModal')">&times;</span>
        </div>
        <div class="modal-body">
            <form method="POST" action="" onsubmit="return validateForm('receiveribbonForm')" id="receiveribbonForm">
                <input type="hidden" name="action" value="receive">
                
                <!-- ribbon Information -->
                <div class="form-section">
                    <h4><i class="fas fa-file"></i> ribbon Information</h4>
                    <div class="form-row">
                        <div class="form-col">
                            <label class="form-label">ribbon Type <span class="required">*</span></label>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-col" style="display: none;">
                            ...
                            <input type="text" name="stock" class="form-control" placeholder="e.g., JCT/UCT">
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-actions">
                    <button type="button" class="btn btn-secondary" onclick="closeModal('receiveribbonModal')">Cancel</button>
                    <button type="submit" class="btn btn-primary" data-original-text="Receive ribbon">
                        <i class="fas fa-truck-loading"></i> Receive ribbon
                    </button>
                </div>
            </form>
        </div>
...
<div id="editReceivingModal" class="modal">
    <div class="modal-content modal-large">
        <div class="modal-header">
            <h2><i class="fas fa-edit"></i> Edit ribbon Receiving</h2>
        ...
        <div class="modal-body">
            <form method="POST" action="" onsubmit="return validateForm('editReceivingForm')" id="editReceivingForm">
                ...
                <div class="form-section">
                    ...
                    <div class="form-row">
                        <div class="form-col">
                            ...
                            <div class="autocomplete-container" style="position: relative;">
                                <input type="text" 
                                       id="editribbonSearchInput" 
                                       class="form-control" 
                                       placeholder="Search for ribbon type..." 
                                       autocomplete="off"
                                       oninput="searchEditribbons(this.value)"
                                       onfocus="searchEditribbons(this.value)">
                                <div id="editribbonSuggestions" class="autocomplete-suggestions"></div>
                                <input type="hidden" name="ribbon_id" id="editribbonselect" required>
                                <input type="hidden" name="ribbon_model" id="editribbonTypeHidden">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-col" style="display: none;">
                            <label class="form-label">Stock Location</label>
                            <input type="text" name="stock" id="editStock" class="form-control" placeholder="e.g., JCT/UCT">
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-actions">
                    ...
                    <button type="submit" class="btn btn-primary" data-original-text="Update ribbon">
