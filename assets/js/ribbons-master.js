// Ribbons Master JavaScript - Exact Copy of Toner Master Functionality
...
function filterTable() {
    ...
    const printerFilter = document.getElementById('printerFilter').value.toLowerCase();
    const locationFilter = document.getElementById('locationFilter').value.toLowerCase();
    
    const table = document.getElementById('RibbonTable');
    if (!table) return;
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (cells.length > 0) {
            const ribbonModel = cells[0].textContent.toLowerCase();
            const compatiblePrinters = cells[1].textContent.toLowerCase();
            const jctStock = parseInt(cells[2].textContent.trim()) || 0;
            const uctStock = parseInt(cells[3].textContent.trim()) || 0;
            
            const matchesSearch = ribbonModel.includes(searchInput) || compatiblePrinters.includes(searchInput);
            const matchesModel = !modelFilter || ribbonModel.includes(modelFilter) || compatiblePrinters.includes(modelFilter);
            const matchesPrinter = !printerFilter || ribbonModel.includes(printerFilter) || compatiblePrinters.includes(printerFilter);
            ...
            if (matchesSearch && matchesModel && matchesPrinter && matchesLocation) {
                row.style.display = '';
            } else {
...
// View ribbon details - Exact Toner Master Style
function viewribbon(index) {
    if (typeof ribbonsData === 'undefined' || !ribbonsData[index]) {
        alert('ribbon data not found!');
        return;
    }
    
    const ribbon = ribbonsData[index];
    const totalStock = (ribbon.jct_stock || 0) + (ribbon.uct_stock || 0);
    
    // Fill modal with data
    document.getElementById('view_ribbon_model').textContent = ribbon.ribbon_model || 'N/A';
    document.getElementById('view_compatible_printers').textContent = ribbon.compatible_printers || 'N/A';
    
    // Stock badges with colored backgrounds
    document.getElementById('view_jct_stock').textContent = 'JCT: ' + (ribbon.jct_stock || 0);
    document.getElementById('view_uct_stock').textContent = 'UCT: ' + (ribbon.uct_stock || 0);
    document.getElementById('view_total_stock').textContent = 'Total: ' + totalStock;
    
    // Purchase date formatting
    if (ribbon.purchase_date) {
        const purchaseDate = new Date(ribbon.purchase_date);
    ...
    // Reorder level
    document.getElementById('view_reorder_level').textContent = ribbon.reorder_level || 0;
    ...
    } else if (totalStock <= (ribbon.reorder_level || 0)) {
...
function openEditModal(index) {
    ...
    document.getElementById('edit_ribbon_model').value = ribbon.ribbon_model || '';
    document.getElementById('edit_compatible_printers').value = ribbon.compatible_printers || '';
    document.getElementById('edit_reorder_level').value = ribbon.reorder_level || '';
    document.getElementById('edit_jct_stock').value = ribbon.jct_stock || '';
    document.getElementById('edit_uct_stock').value = ribbon.uct_stock || '';
    document.getElementById('edit_purchase_date').value = ribbon.purchase_date || '';
    
    openModal('editRibbonModal');
}
...
function generatePrintReport() {
    ...
    const printUrl = 'ribbons_master_print.php?' + params.toString();
