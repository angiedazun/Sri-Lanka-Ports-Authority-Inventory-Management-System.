// Ribbons Receiving Page JavaScript Functions

document.addEventListener('DOMContentLoaded', function() {
    initializeribbonsReceiving();
});

function initializeribbonsReceiving() {
    ...
    // Initialize ribbon selection functionality
    initializeribbonselection();
...
function filterTable() {
    ...
    Array.from(rows).forEach(row => {
        ...
        // Columns: Receive Date, ribbon Type, LOT, Supplier Name, PR No, Tender File No, JCT Quantity, UCT Quantity, Unit Price, Invoice, Remarks, Actions
        const receiveDate = cells[0].textContent.toLowerCase();
        const ribbonType = cells[1].textContent.toLowerCase();
        ...
        const tenderFileNo = cells[5].textContent.toLowerCase();
        const invoice = cells[9].textContent.toLowerCase();
        const remarks = cells[10].textContent.toLowerCase();
        
        // Check search criteria
        const matchesSearch = searchValue === '' || 
            ribbonType.includes(searchValue) || 
            lot.includes(searchValue) ||
            supplier.includes(searchValue) || 
            prNo.includes(searchValue) ||
            tenderFileNo.includes(searchValue) ||
        ...
        // For stock filter, check if row has JCT or UCT quantities
        let matchesStock = true;
        if (stockValue !== '') {
            const jctQty = parseInt(cells[6].textContent) || 0;
            const uctQty = parseInt(cells[7].textContent) || 0;
            
            if (stockValue === 'jct') {
                matchesStock = jctQty > 0;
            } else if (stockValue === 'uct') {
                matchesStock = uctQty > 0;
            } else if (stockValue === 'both') {
                matchesStock = jctQty > 0 && uctQty > 0;
            }
        }
...
// ribbon Selection Functions
function initializeribbonselection() {
    // Initialize autocomplete for ribbon search
    initializeAutocomplete();
    
    // Close suggestions when clicking outside
    document.addEventListener('click', function(event) {
        if (!event.target.closest('.autocomplete-container')) {
            const suggestionDivs = document.querySelectorAll('.autocomplete-suggestions');
            suggestionDivs.forEach(div => div.style.display = 'none');
        }
    });
}

// Autocomplete functionality for ribbon search (Add form)
function searchribbons(query) {
    const suggestionsDiv = document.getElementById('ribbonSuggestions');
    if (!suggestionsDiv) return;
    
    query = query.toLowerCase().trim();
    
    if (query.length === 0) {
        // Show all ribbons when field is focused but empty
        displayribbonSuggestions(ribbonsData, suggestionsDiv);
        return;
    }
    
    // Filter ribbons based on search query
    const filtered = ribbonsData.filter(ribbon => {
        const model = ribbon.ribbon_model.toLowerCase();
        const id = ribbon.ribbon_id.toString();
        return model.includes(query) || id.includes(query);
    });
    
    displayribbonSuggestions(filtered, suggestionsDiv);
}

// Display ribbon suggestions with professional design
function displayribbonSuggestions(ribbons, suggestionsDiv) {
    if (ribbons.length === 0) {
        suggestionsDiv.innerHTML = `
            <div class="no-suggestions">
                <i class="fas fa-search"></i>
                <p>No ribbons found</p>
    ...
    ribbons.forEach(ribbon => {
        html += `
            <div class="suggestion-item" onclick="selectribbon(${ribbon.ribbon_id}, '${escapeHtml(ribbon.ribbon_model)}')">
                <div class="suggestion-code">
                    <i class="fas fa-barcode"></i> ${escapeHtml(ribbon.ribbon_model)}
                </div>
                <div class="suggestion-details">
                    <span class="detail-badge badge-id">ID: ${ribbon.ribbon_id}</span>
                </div>
            </div>
        `;
    });
...
// Select ribbon from autocomplete (Add form)
function selectribbon(ribbonId, ribbonModel) {
    // Set hidden fields
    document.getElementById('ribbonselect').value = ribbonId;
    document.getElementById('ribbonTypeHidden').value = ribbonModel;
    
    // Set search input display text
    document.getElementById('ribbonSearchInput').value = ribbonModel;
    
    // Hide suggestions
    document.getElementById('ribbonSuggestions').style.display = 'none';
    
    // Update ribbon details if needed
    updateribbonDetails();
}

// Autocomplete functionality for ribbon search (Edit form)
function searchEditribbons(query) {
    const suggestionsDiv = document.getElementById('editribbonSuggestions');
    if (!suggestionsDiv) return;
    ...
    if (query.length === 0) {
        // Show all ribbons when field is focused but empty
        displayEditribbonSuggestions(ribbonsData, suggestionsDiv);
        return;
    }
    ...
    displayEditribbonSuggestions(filtered, suggestionsDiv);
}

// Display ribbon suggestions for edit form
function displayEditribbonSuggestions(ribbons, suggestionsDiv) {
    if (ribbons.length === 0) {
        suggestionsDiv.innerHTML = `
            <div class="no-suggestions">
                <i class="fas fa-search"></i>
    ...
    ribbons.forEach(ribbon => {
        html += `
            <div class="suggestion-item" onclick="selectEditribbon(${ribbon.ribbon_id}, '${escapeHtml(ribbon.ribbon_model)}')">
                <div class="suggestion-code">
                    <i class="fas fa-barcode"></i> ${escapeHtml(ribbon.ribbon_model)}
                </div>
                <div class="suggestion-details">
...
// Select ribbon from autocomplete (Edit form)
function selectEditribbon(ribbonId, ribbonModel) {
    // Set hidden fields
    document.getElementById('editribbonselect').value = ribbonId;
    document.getElementById('editribbonTypeHidden').value = ribbonModel;
    
    // Set search input display text
    document.getElementById('editribbonSearchInput').value = ribbonModel;
    
    // Hide suggestions
    document.getElementById('editribbonSuggestions').style.display = 'none';
    
    // Update ribbon details if needed
    updateEditribbonDetails();
}

// Initialize autocomplete functionality
function initializeAutocomplete() {
    // Add keyboard navigation for autocomplete
    const searchInputs = document.querySelectorAll('#ribbonSearchInput, #editribbonSearchInput');
    searchInputs.forEach(input => {
        input.addEventListener('keydown', handleAutocompleteKeyboard);
    });
}
...
function updateribbonDetails() {
    // Keep for compatibility - ribbon details are now set directly in selectribbon
    const ribbonId = document.getElementById('ribbonselect').value;
    const ribbonModel = document.getElementById('ribbonTypeHidden').value;
    
    // You can add additional logic here if needed when ribbon is selected
    console.log('Selected ribbon:', ribbonId, ribbonModel);
}

function updateEditribbonDetails() {
    // Keep for compatibility - ribbon details are now set directly in selectEditribbon
    const ribbonId = document.getElementById('editribbonselect').value;
    const ribbonModel = document.getElementById('editribbonTypeHidden').value;
    
    // You can add additional logic here if needed when ribbon is selected
    console.log('Selected ribbon for edit:', ribbonId, ribbonModel);
}

// Stock preview functions
function updateStockPreview() {
    const stockInput = document.getElementById('stockInput');
    const stockPreview = document.getElementById('stockPreview');
    const stockPreviewText = document.getElementById('stockPreviewText');
    
    if (stockInput && stockPreview && stockPreviewText) {
        const stockNumber = parseInt(stockInput.value) || 0;
        if (stockNumber > 0) {
            stockPreviewText.textContent = 'LOT' + stockNumber;
...
function validateForm(formId) {
    ...
    // Update LOT preview before submission
    if (formId === 'receiveribbonForm') {
        updateLotPreview();
    } else if (formId === 'editReceivingForm') {
        updateEditLotPreview();
    }
        ...
        <div class="view-section">
            ...
            <div class="view-grid">
                <div class="view-item">
                    ...
                    <span>${receiving.ribbon_type}</span>
                </div>
                <div class="view-item">
                    <label>LOT:</label>
                    <span class="stock-badge" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 4px 12px; border-radius: 15px; font-weight: 600;">${receiving.lot || 'N/A'}</span>
                </div>
            </div>
        </div>
        ...
        <div class="view-section">
            ...
            <div class="view-grid">
                <div class="view-item">
                    ...
                    <span class="quantity-badge">${receiving.jct_quantity} bundles</span>
                </div>
                <div class="view-item">
                    <label>UCT Quantity:</label>
                    <span class="quantity-badge">${receiving.uct_quantity} bundles</span>
                </div>
                <div class="view-item">
                    <label>Total Quantity:</label>
                    <span class="quantity-badge">${parseInt(receiving.jct_quantity) + parseInt(receiving.uct_quantity)} bundles</span>
                </div>
                <div class="view-item">
...
function editReceiving(receiveId) {
    ...
    setFieldValue('editribbonselect', receiving.ribbon_id);
    setFieldValue('editribbonTypeHidden', receiving.ribbon_model);
    
    // Set the autocomplete search input to display the ribbon model
    setFieldValue('editribbonSearchInput', receiving.ribbon_model);
    
    // Extract LOT number from format (e.g., "2025/LOT1" -> "1" or "LOT1" -> "1")
    let lotNumber = '';
    if (receiving.lot) {
        lotNumber = receiving.lot.replace(/^\d{4}\/LOT/, '').replace('LOT', '');
    }
    setFieldValue('editLotNumber', lotNumber);
    setFieldValue('editLotHidden', receiving.lot);
    ...
    // Trigger the ribbon details update
    updateEditribbonDetails();
...
function deleteReceiving(receiveId) {
    if (confirm('Are you sure you want to delete this receiving record? This action cannot be undone and will adjust the stock levels.')) {
        ...
        form.method = 'POST';
        form.action = 'ribbons_receiving.php';
        ...
        idInput.type = 'hidden';
        idInput.name = 'receive_id';
        idInput.value = receiveId;
...
function printTable() {
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Ribbons Receivings Report</title>
            <style>
        ...
        <body>
            <h2>Ribbons Receivings Report</h2>
...
function exportToCSV() {
    ...
    a.href = url;
    a.download = `ribbons_receivings_${new Date().toISOString().split('T')[0]}.csv`;
...
function updateStockValue(number, hiddenField) {
    if (number && parseInt(number) > 0) {
        hiddenField.value = 'LOT ' + parseInt(number);
    } else {
        hiddenField.value = '';
    }
}
...
function generatePrintReport() {
    ...
    let reportTitle = 'Ribbons Receiving Report';
    let dateRange = '';
    ...
    switch(printType) {
        case 'daily':
            ...
            if (dailyDate) {
                ...
                reportTitle = 'Daily Ribbons Receiving Report';
        ...
        case 'monthly':
            ...
            reportTitle = 'Monthly Ribbons Receiving Report';
        ...
        case 'yearly':
            ...
            reportTitle = 'Yearly Ribbons Receiving Report';
            dateRange = year;
            break;
        ...
        case 'custom':
            ...
            if (fromDate && toDate) {
                ...
                reportTitle = 'Custom Ribbons Receiving Report';
        ...
        case 'all':
        default:
            reportTitle = 'Complete Ribbons Receiving Report';
...
function generatePrintHTML(title, dateRange, data, format, options) {
    ...
    if (format !== 'statistics') {
        ...
        let columns = [
            ...
            { key: 'ribbon_type', label: 'ribbon TYPE', show: true },
            { key: 'lot', label: 'LOT', show: true },
                ...
                if (col.show) {
                    ...
                    switch(col.key) {
                        case 'receive_date':
                            ...
                            break;
                        case 'ribbon_type':
                            cellValue = item.ribbon_type || 'N/A';
                            break;
                        case 'lot':
                            cellValue = item.lot || 'N/A';
                            break;
                        case 'supplier_name':
                            cellValue = item.supplier_name || 'N/A';
                            break;
                        case 'pr_no':
                            cellValue = item.pr_no || 'N/A';
                            break;
                        case 'tender_file_no':
                            cellValue = item.tender_file_no || 'N/A';
                            break;
                        case 'jct_quantity':
    ...
    html += `
            <div class="footer">
                <p>This report was generated by SLPA ribbons Management System</p>
...
// Full Payment Calculation for Receive Form
function calculateFullPayment() {
    const unitPriceInput = document.getElementById('unitPrice');
    const unitPrice = parseFloat(unitPriceInput?.value) || 0;
    const jctQty = parseInt(document.getElementById('jctQuantity')?.value) || 0;
    const uctQty = parseInt(document.getElementById('uctQuantity')?.value) || 0;
    const validationDiv = document.getElementById('unitPriceValidation');
    
    if (!validationDiv || !unitPriceInput) return;
    
    if (unitPrice > 0) {
        const totalQty = jctQty + uctQty;
        const fullPayment = unitPrice * totalQty;
        
        // Add green border to input
        unitPriceInput.style.borderColor = '#28a745';
        unitPriceInput.style.borderWidth = '2px';
        
        validationDiv.innerHTML = `
            <div style="background-color: #d4edda; border: 2px solid #c3e6cb; border-radius: 6px; padding: 12px; margin-top: 8px;">
                <div style="color: #2d5f2e; font-size: 0.95em; margin-bottom: 8px;">
                    <span style="display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; background-color: #28a745; color: white; border-radius: 50%; font-size: 0.75em; margin-right: 6px; font-weight: bold;">?</span>
                    <strong>Unit price: Rs. ${unitPrice.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})} - Valid</strong>
                </div>
                <div style="color: #2d5f2e; font-weight: bold; font-size: 1.05em; margin-bottom: 6px;">
                    <span style="display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; background-color: #dc3545; color: white; border-radius: 50%; font-size: 0.75em; margin-right: 6px; font-weight: bold;">?</span>
                    FULL PAYMENT: Rs. ${fullPayment.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                </div>
                <div style="color: #5a7c5b; font-size: 0.85em;">
                    Calculation: ${jctQty} JCT + ${uctQty} UCT � Rs. ${unitPrice.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})} = Rs. ${fullPayment.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                </div>
            </div>
        `;
    } else {
        ...
        unitPriceInput.style.borderWidth = '';
        validationDiv.innerHTML = '';
    }
}

// Full Payment Calculation for Edit Form
function calculateEditFullPayment() {
    const unitPriceInput = document.getElementById('editUnitPrice');
    const unitPrice = parseFloat(unitPriceInput?.value) || 0;
    const jctQty = parseInt(document.getElementById('editJctQuantity')?.value) || 0;
    const uctQty = parseInt(document.getElementById('editUctQuantity')?.value) || 0;
    const validationDiv = document.getElementById('editUnitPriceValidation');
...
// Update Edit ribbon Details
function updateEditribbonDetails() {
    const ribbonselect = document.getElementById('editribbonselect');
    if (!ribbonselect) return;
    
    const selectedOption = ribbonselect.options[ribbonselect.selectedIndex];
    ...
    setFieldValue('editribbonTypeDisplay', selectedOption.getAttribute('data-type'));
    setFieldValue('editribbonsizeDisplay', selectedOption.getAttribute('data-size'));
    setFieldValue('editGsmDisplay', selectedOption.getAttribute('data-gsm'));
    setFieldValue('editSheetsDisplay', selectedOption.getAttribute('data-sheets'));
}

// LOT Number change handler for edit form
function updateEditLotPreview() {
    const lotInput = document.getElementById('editLotNumber');
    const lotHidden = document.getElementById('editLotHidden');
    
    if (lotInput && lotHidden) {
        const lotValue = lotInput.value.trim();
        if (lotValue) {
            const currentYear = new Date().getFullYear();
            lotHidden.value = currentYear + '/LOT' + lotValue;
        } else {
            lotHidden.value = '';
        }
    }
...
// LOT Number change handler for receive form
...
function confirmDelete(receiveId) {
    if (confirm('Are you sure you want to delete this receiving record?\\n\\nThis action cannot be undone and will adjust the stock accordingly.')) {
        window.location.href = '?delete=' + receiveId;
    }
}

// Calculate payment for add form
function calculatePayment() {
    const jctQty = parseFloat(document.getElementById('jctQuantity').value) || 0;
    const uctQty = parseFloat(document.getElementById('uctQuantity').value) || 0;
    const unitPrice = parseFloat(document.getElementById('unitPrice').value) || 0;
    
    const totalQty = jctQty + uctQty;
    const fullPayment = totalQty * unitPrice;
    
    const paymentDiv = document.getElementById('paymentCalculation');
    
    if (unitPrice > 0 && totalQty > 0) {
        paymentDiv.style.display = 'block';
        document.getElementById('displayUnitPrice').textContent = unitPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById('displayFullPayment').textContent = fullPayment.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById('displayCalculation').textContent = totalQty + ' � Rs. ' + unitPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' = Rs. ' + fullPayment.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    } else {
        paymentDiv.style.display = 'none';
    }
}

// Calculate payment for edit form
function calculateEditPayment() {
    const jctQty = parseFloat(document.getElementById('editJctQuantity').value) || 0;
    const uctQty = parseFloat(document.getElementById('editUctQuantity').value) || 0;
    const unitPrice = parseFloat(document.getElementById('editUnitPrice').value) || 0;
    
    const totalQty = jctQty + uctQty;
    const fullPayment = totalQty * unitPrice;
    
    const paymentDiv = document.getElementById('editPaymentCalculation');
    
    if (unitPrice > 0 && totalQty > 0) {
        paymentDiv.style.display = 'block';
        document.getElementById('editDisplayUnitPrice').textContent = unitPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById('editDisplayFullPayment').textContent = fullPayment.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById('editDisplayCalculation').textContent = totalQty + ' � Rs. ' + unitPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' = Rs. ' + fullPayment.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    } else {
        paymentDiv.style.display = 'none';
    }
}
