// Papers Receiving Page JavaScript Functions

document.addEventListener('DOMContentLoaded', function() {
    initializePapersReceiving();
});

function initializePapersReceiving() {
    ...
    // Initialize paper selection functionality
    initializePaperSelection();
...
function filterTable() {
    ...
    Array.from(rows).forEach(row => {
        ...
        // Columns: Receive Date, Paper Type, LOT, Supplier Name, PR No, Tender File No, JCT Quantity, UCT Quantity, Unit Price, Invoice, Remarks, Actions
        const receiveDate = cells[0].textContent.toLowerCase();
        const paperType = cells[1].textContent.toLowerCase();
        ...
        const matchesSearch = searchValue === '' || 
            paperType.includes(searchValue) || 
...
function initializePaperSelection() {
    const paperSelect = document.getElementById('paperSelect');
    if (paperSelect) {
        paperSelect.addEventListener('change', updatePaperDetails);
    }
    
    const editPaperSelect = document.getElementById('editPaperSelect');
    if (editPaperSelect) {
        editPaperSelect.addEventListener('change', updateEditPaperDetails);
    }
}
...
function updatePaperDetails() {
    ...
    if (selectedOption.value) {
        const paperType = selectedOption.getAttribute('data-type');
        const hiddenField = document.getElementById('paperTypeHidden');
        if (hiddenField) {
            hiddenField.value = paperType || '';
        }
    } else {
        const hiddenField = document.getElementById('paperTypeHidden');
        if (hiddenField) {
...
function updateEditPaperDetails() {
    const select = document.getElementById('editPaperSelect');
    const option = select.options[select.selectedIndex];
    
    if (option.value) {
        const hiddenField = document.getElementById('editPaperTypeHidden');
        if (hiddenField) {
            hiddenField.value = option.getAttribute('data-type') || '';
        }
    } else {
        const hiddenField = document.getElementById('editPaperTypeHidden');
        if (hiddenField) {
        ...
        <div class="view-section">
            ...
            <div class="view-grid">
                <div class="view-item">
                    ...
                    <span>${receiving.paper_type}</span>
                </div>
                <div class="view-item">
                    <label>LOT:</label>
...
function editReceiving(receiveId) {
    ...
    setFieldValue('editPaperSelect', receiving.paper_id);
    setFieldValue('editPaperTypeHidden', receiving.paper_type);
    ...
    // Trigger the paper details update
    updateEditPaperDetails();
...
function deleteReceiving(receiveId) {
    if (confirm('Are you sure you want to delete this receiving record? This action cannot be undone and will adjust the stock levels.')) {
        ...
        form.method = 'POST';
        form.action = 'papers_receiving.php';
...
function printTable() {
    ...
    printWindow.document.write(`
        <html>
        <head>
            <title>Papers Receivings Report</title>
            <style>
        ...
        <body>
            <h2>Papers Receivings Report</h2>
...
function exportToCSV() {
    ...
    a.href = url;
    a.download = `papers_receivings_${new Date().toISOString().split('T')[0]}.csv`;
...
function generatePrintReport() {
    ...
    let reportTitle = 'Papers Receiving Report';
    let dateRange = '';
    ...
    switch(printType) {
        case 'daily':
            ...
            if (dailyDate) {
                ...
                reportTitle = 'Daily Papers Receiving Report';
        ...
        case 'monthly':
            ...
            reportTitle = 'Monthly Papers Receiving Report';
        ...
        case 'yearly':
            ...
            reportTitle = 'Yearly Papers Receiving Report';
            dateRange = year;
            break;
        ...
        case 'custom':
            ...
            if (fromDate && toDate) {
                ...
                reportTitle = 'Custom Papers Receiving Report';
        ...
        case 'all':
        default:
            reportTitle = 'Complete Papers Receiving Report';
...
function generatePrintHTML(title, dateRange, data, format, options) {
    ...
    if (format !== 'statistics') {
        ...
        let columns = [
            ...
            { key: 'paper_type', label: 'PAPER TYPE', show: true },
    ...
    html += `
            <div class="footer">
                <p>This report was generated by SLPA Papers Management System</p>
...
function updateEditPaperDetails() {
    ...
    setFieldValue('editPaperTypeDisplay', selectedOption.getAttribute('data-type'));
    setFieldValue('editPaperSizeDisplay', selectedOption.getAttribute('data-size'));
