// Ribbons Issuing Page JavaScript Functions

// Store ribbon data globally
let selectedRibbonData = null;

document.addEventListener('DOMContentLoaded', function() {
    console.log('=== RIBBONS-ISSUING.JS LOADED ===');
    initializeRibbonsIssuing();
});

function initializeRibbonsIssuing() {
...
// Update ribbon details when selected
function updateRibbonDetails() {
    ...
    if (selectedOption && selectedOption.value) {
        selectedRibbonData = {
            ribbon_id: selectedOption.value,
            ribbon_model: selectedOption.dataset.model || '',
        ...
        console.log('✅ Ribbon selected:', selectedRibbonData);
        
        // Update display fields
        const ribbonModelDisplay = document.getElementById('ribbonModelDisplay');
        const lotDisplay = document.getElementById('lotDisplay');
        
        if (ribbonModelDisplay) ribbonModelDisplay.value = selectedRibbonData.ribbon_model;
        if (lotDisplay) lotDisplay.value = selectedRibbonData.lot;
        ...
        if (stockInfoBox && jctQty && uctQty && totalQty) {
            jctQty.textContent = selectedRibbonData.jct_quantity;
            uctQty.textContent = selectedRibbonData.uct_quantity;
            totalQty.textContent = selectedRibbonData.jct_quantity + selectedRibbonData.uct_quantity;
            
            // Show with animation
            stockInfoBox.style.display = 'flex';
            console.log('📦 Stock info displayed - JCT:', selectedRibbonData.jct_quantity, 'UCT:', selectedRibbonData.uct_quantity);
        }
        ...
        console.log('⚠️ No ribbon selected - hiding stock info');
        
        selectedRibbonData = null;
        
        const ribbonModelDisplay = document.getElementById('ribbonModelDisplay');
        const lotDisplay = document.getElementById('lotDisplay');
        const codeDisplay = document.getElementById('codeDisplay');
        
        if (ribbonModelDisplay) ribbonModelDisplay.value = '';
...
function updateAvailableStock() {
    if (!selectedRibbonData) {
        alert('Please select a ribbon first!');
    ...
    if (stockSelect.value === 'JCT') {
        maxStock = selectedRibbonData.jct_quantity;
    } else if (stockSelect.value === 'UCT') {
        maxStock = selectedRibbonData.uct_quantity;
    }
...
function openModal(modalId) {
    ...
    if (modal) {
        ...
        if (modalId === 'issueRibbonModal') {
            const ribbonSelect = document.getElementById('ribbonSelect');
            const stockSelect = document.getElementById('stockSelect');
            const stockInfoBox = document.getElementById('stockInfoBox');
            
            if (ribbonSelect) ribbonSelect.selectedIndex = 0;
            if (stockSelect) stockSelect.selectedIndex = 0;
...
function editIssue(issue) {
    ...
    document.getElementById('editRibbonId').value = issue.ribbon_id || '';
    document.getElementById('editRibbonModel').value = issue.ribbon_model || '';
...
function confirmDelete(issueId) {
    if (confirm('Are you sure you want to delete this ribbon issue record? This action cannot be undone.')) {
...
function viewIssue(issue) {
    let details = `
        <div style="padding: 20px;">
            <h3>Ribbon Issue Details</h3>
            <table style="width: 100%; border-collapse: collapse;">
                <tr><td style="padding: 8px; font-weight: bold;">Issue Date:</td><td style="padding: 8px;">${issue.issue_date}</td></tr>
                <tr><td style="padding: 8px; font-weight: bold;">Ribbon Model:</td><td style="padding: 8px;">${issue.ribbon_model || 'N/A'}</td></tr>
    ...
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                <h2><i class="fas fa-info-circle"></i> View Ribbon Issue</h2>
...
function exportToCSV() {
    ...
    a.href = url;
    a.download = 'ribbons_issuing_' + new Date().toISOString().split('T')[0] + '.csv';
    a.click();
